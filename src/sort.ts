...
function createCompareFn<T>(order: SortOrder): CompareFn<T> {
  if (order === "asc") {
    return (a, b) => {
      if (a > b) return 1;
...
export function simpleSort<T>(arr: T[], order: SortOrder = "asc"): T[] {
  const filteredArr = arr.filter(isNonNullish);

  const compare: CompareFn<T> = createCompareFn(order);
  const arrSize = filteredArr.length;

  for (let i = 0; i < arrSize - 1; i++) {
    for (let j = 0; j < arrSize - 1; j++) {
      let curr: T | undefined = filteredArr[j];
      let next: T | undefined = filteredArr[j + 1];
      if (curr !== undefined && next !== undefined) {
        if (compare(curr, next) > 0) {
          [filteredArr[j], filteredArr[j + 1]] = [next, curr];
        }
      }
    }
  }
